...
<div x-data="{
    ...
    <div @click="isOpen = !isOpen"
        class="min-h-[40px] w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-2 flex flex-wrap gap-1 items-center cursor-pointer shadow-sm hover:border-zinc-300 dark:hover:border-zinc-700   s">
        ...
        <template x-for="val in selected" :key="val">
            <span
                class="flex items-center gap-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 text-xs px-2 py-1 rounded-md font-medium border border-indigo-100 dark:border-indigo-800/50">
                <span x-text="options.find(o => o.value == val)?.label || val"></span>
                <button type="button" @click.stop="remove(val)"
                    class="hover:text-indigo-900 dark:hover:text-indigo-200   s">
    ...
    <div x-show="isOpen" x-transition.opacity.duration.200ms
        ...
        <div class="p-2 border-b border-zinc-100 dark:border-zinc-800">
            <input type="text" x-model="search" placeholder="ابحث..."
        ...
        <ul class="max-h-60 overflow-y-auto p-1 text-sm">
            <template x-for="option in filteredOptions" :key="option.value">
                <li @click="toggle(option.value)"
                    class="px-3 py-2 flex items-center gap-2 cursor-pointer rounded-md   s"
                    ...
                    <div class="size-4 border rounded shrink-0 flex items-center justify-center   s"
