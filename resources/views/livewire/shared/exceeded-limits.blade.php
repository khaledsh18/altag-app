            ...
            <flux:table class="w-full">
                ...
                <flux:table.rows>
                    @foreach ($students as $student)
                        <flux:table.row :key="$student->id" class="{{ $student->recent_absences_count >= $absenceLimit ? 'bg-red-50/50 dark:bg-red-900/10' : '' }}">
                            <flux:table.cell>
                            ...
                            <flux:table.cell class="text-center">
                                @if ($student->recent_lateness_count >= $latenessLimit)
                                    <span class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-amber-900 bg-amber-400 rounded-full">
                            ...
                            <flux:table.cell class="text-center">
                                ...
                                @if ($student->guardian && $student->guardian->phone)
                                    <a class="inline-flex items-center justify-center p-2 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition-colors"
                                ...
                                @else
                                    <button x-data="{ copied: false }" 
                                            ...
                                            class="inline-flex items-center justify-center p-2 rounded-lg bg-zinc-50 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors"
